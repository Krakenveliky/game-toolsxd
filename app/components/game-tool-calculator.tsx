...
import { gameConfigs } from "../lib/game-tools/config";
import type { GameConfig, GameId, ToolId } from "../lib/game-tools/types";

type Props = { game: GameConfig; toolId: ToolId };
...
export function GameToolCalculator({ game, toolId }: Props) {
  const [values, setValues] = useState<Values>({ ...initialValues, targetGame: game.id === "valorant" ? "cs2" : "valorant" });
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const numericKeys = fields.map(([key]) => key);
    const inputs = numericKeys.map((key) => values[key]);
    ...
    if (toolId === "sensitivity-converter") {
      ...
      const targetGame = values.targetGame as GameId;
      const targetYaw = gameConfigs[targetGame].yawDegreesPerCount;
      if (!targetYaw) { setError("The selected game does not have a verified sensitivity conversion yet."); return; }
      setResult(convertSensitivity(number("sensitivity"), game.yawDegreesPerCount, targetYaw).toFixed(4));
